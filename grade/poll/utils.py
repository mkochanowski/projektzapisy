...
def group_polls_and_tickets_by_subject( poll_and_ticket_list ):
    ...
    poll_and_ticket_list.sort( lambda (p1, t1), (p2, t2): poll_cmp( p1, p2 ))
    
    res       = []
    act_polls = []
    act_group = poll_and_ticket_list[ 0 ][ 0 ].group
    
    for ( poll, ticket ) in poll_and_ticket_list:
        if   not act_group:
            if   poll.group == act_group:
                act_polls.append(( poll.pk, ticket ))
            else:
                res.append(( u'Ankiety ogólne', act_polls ))
                act_group = poll.group
                act_polls = [( poll.pk, ticket )]
        else:
