...
class Term(models.Model):
    ...
    def get_all_in_semester(semester, student=None, employee=None):
        ...
        if employee:
            from apps.enrollment.records.models import Record
            filtered = filtered.filter(group__teachers__in=[employee])

        return filtered.select_related(
            'classroom', 'group', 'group__course', 'group__course__semester',
            'group__course__entity', 'group__course__entity__type'
        ).prefetch_related(
            'classrooms', 'group__teachers', 'group__teachers__user'
