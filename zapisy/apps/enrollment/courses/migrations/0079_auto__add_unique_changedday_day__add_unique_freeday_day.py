...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding unique constraint on 'ChangedDay', fields ['day']
        # REMOVE DUPLICATES
        db.execute("DELETE FROM courses_changedday WHERE id NOT IN (SELECT MIN(id) FROM courses_changedday GROUP BY day)")
        db.create_unique('courses_changedday', ['day'])

        # Adding unique constraint on 'Freeday', fields ['day']
        # REMOVE DUPLICATES
        db.execute("DELETE FROM courses_freeday WHERE id NOT IN (SELECT MIN(id) FROM courses_freeday GROUP BY day)")
        db.create_unique('courses_freeday', ['day'])


    def backwards(self, orm):
        # Removing unique constraint on 'Freeday', fields ['day']
        db.delete_unique('courses_freeday', ['day'])

        # Removing unique constraint on 'ChangedDay', fields ['day']
        db.delete_unique('courses_changedday', ['day'])
    ...
    models = {
        ...
        'users.employee': {
            ...
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'last_news_view': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2017, 4, 7, 0, 0)'}),
