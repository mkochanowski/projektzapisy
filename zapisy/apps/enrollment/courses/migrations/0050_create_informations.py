...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for course in orm.Course.objects.all():
            desc = orm.CourseDescription()
            desc.exam = course.exam
            desc.entity = course.entity
            desc.is_ready = True
            desc.type = course.type
            desc.description = course.description
            if course.teachers.all():
                desc.author = course.teachers.all()[0]
            else:
                desc.author = orm['users.Employee'].objects.all()[0]
            desc.save()

            for r in course.requirements.all():
                desc.requirements.add(r)
