...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        db.add_column('courses_classroom', 'slug',
                      self.gf('autoslug.fields.AutoSlugField')(default=' ', unique_with=('number',), max_length=50, populate_from='number'),
                      keep_default=False)
        from autoslug.settings import slugify
        for room in orm.Classroom.objects.all():
            room.slug = slugify(room.number)
            room.save()


    def backwards(self, orm):
        # Deleting field 'Classroom.slug'
        db.delete_column('courses_classroom', 'slug')
