...
XSCHEMA = os.path.join(settings.BASE_DIR,
                       'enrollment/courses/admin/xml/semesterschedule.xsd')
...
def import_semester_schedule(xmlfile):
    ...
    def get_teacher(el_teacher):
        ...
        try:
            employee = Employee.objects.get(first_name=name, last_name=surname)
        except Employee.DoesNotExist:
            raise Employee.DoesNotExist(
                'Employee name="%s"  surname="%s" does not exist' % (name,
                                                                     surname))
    ...
    def create_course(el_course, semester):
        ...
        # TODO: nie testowane:
        sub_type = Type.objects.get_or_create(
            name=el_course.find('type').text, meta_type=False)[0]
    ...
    def create_groups(el_groups, course):
        ...
        for el_group in el_groups:
            ...
            group = Group.objects.create(
                course=course, type=gr_type, limit=limit)
    ...
    def create_terms(el_terms, group):
        ...
        for el_term in el_terms:
            ...
            classroom = Classroom.objects.get_or_create(
                number=el_term.find('room').text)[0]
    ...
    for event, element in events:
        if event == 'start' and element.tag == 'semester':
            ...
            semester = Semester.objects.create(
                visible=False, year=sem_year, type=sem_type)
