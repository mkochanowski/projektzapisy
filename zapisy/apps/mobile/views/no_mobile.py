...
def onMobile(request):
    """
        Stores information in the session about user's choice to use the apps.mobile version.
    """
    request.mobile = True
    request.session.set_expiry(0)
    ...
    request.urlconf = 'apps.mobile_urls'
    logger.info('User %s switch to apps.mobile fereol' % (request.user))
    return HttpResponseRedirect('/')
